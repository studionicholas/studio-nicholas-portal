        ...
        <div className="flex-1 overflow-y-auto py-2">
          {projectList.map((p) => {
            ...
            return (
              ...
              >
                <div className="min-w-0">
                  <p className="text-[14px] text-stone-800 truncate flex items-center gap-1.5">
                    {p.name}
                    {unread > 0 && <span className="w-1.5 h-1.5 rounded-full bg-[#B7453C] shrink-0" />}
                  </p>
            ...
            <div className="flex flex-wrap gap-1 mb-8 border-b border-stone-200 pb-4">
              {ADMIN_TABS.map((t) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-[13px] transition-colors ${adminTab === t.id ? "bg-stone-900 text-white" : "text-stone-500 hover:bg-stone-100"}`}
                >
                  {t.label}
                </button>
              ))}
